...
| 书架整理 | 自建分组、多分组归属、分组重命名与安全删除、最近阅读、正在阅读 / 快读完 / 搁置较久 / 有标注智能书架 |
| 书籍资料 | 编辑书名、作者、备注、封面，以及仅对单本书生效的阅读背景 |
| 深度阅读 | 章节目录、目录搜索、全文搜索、故事地图、前进 / 后退阅读位置、连续与双页布局 |
| 阅读辅助 | 章节与正文摘要续读记忆、书签、文字标注、带出处复制、阅读标尺、句子级自动滚动高亮 |
| 朗读与统计 | 调用浏览器 / 系统语音、每本书独立声音偏好、阅读时长与剩余时间估算 |
| 阅读洞察 | 可调每日目标、七日节奏、连续达标、按个人速度生成读完计划，以及每日标注回顾 |
...
4. 在“阅读洞察”中调整每日目标、查看预计完成日期，并从每日回顾跳回标注原文。
5. 定期使用“备份书架”导出书籍、阅读记录、偏好与分组信息。
...
当前回归基线覆盖智能书架、续读记忆、故事地图、阅读目标、完成计划、标注回顾、书签、章节进度、全文搜索、自动滚动、语音偏好、元数据编辑、大文件阅读和自建分组等关键流程。
